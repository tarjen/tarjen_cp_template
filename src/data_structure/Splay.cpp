struct Splay {
  ...
  int newnode(int x) {
    cnt++; val[cnt] = x; w[cnt] = rand(); siz[cnt] = 1;
    return cnt;}
  ...
  int merge(int x, int y) {
    if (!x || !y) return x + y;
    if (w[x] < w[y]) {
      c[x][1] = merge(c[x][1], y); update(x);
      return x;
    } else {
      c[y][0] = merge(x, c[y][0]); update(y);
      return y;}
  }
  int th(int i, int v) {
    if (siz[c[i][0]] + 1 == v) return val[i];
    if (v <= siz[c[i][0]]) return th(c[i][0], v);
    else return th(c[i][1], v - siz[c[i][0]] - 1);
  }
  /*-----------------------------------------------*/
  ...
  int queryrk(int v) {  // 定义排名为比当前数小的数的个数 +1 查询 v 的排名。
    ...
    r = merge(x, y);
    return ans;}
  int querynum(int v) { return th(r, v);} // 查询排名为 v 的数
  int query_pre(int v) {  // 求 v 的前驱（前驱定义为小于 v，且最大的数）
    int x, y; split(x, y, r, v - 1);
    int i = x;
    while (c[i][1]) i = c[i][1];
    int ans = val[i]; r = merge(x, y);
    return ans;}
  int query_suf(int v) {  // 求 v 的前驱（前驱定义为小于 v，且最大的数）
    int x, y; split(x, y, r, v);
    int i = y;
    while (c[i][0]) i = c[i][0];
    int ans = val[i]; r = merge(x, y);
    return ans;}
