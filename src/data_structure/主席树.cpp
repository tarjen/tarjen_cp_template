...
struct Persistent_SegmentTree {
        ...
        int r)  // s,t为root[l],root[r]的根节点 中所有大小在[l,r]之间数字的和
    {
        ...
        int res = 0;
        int mid = (L + R) >> 1;
        if (l <= mid) res += getsum(lch[s], lch[t], L, mid, l, r);
...
int main() {
    ...
    while (q--) {
        ...
        if (ops == 2) {  // get kth max no output -1
            cin >> l >> r >> k;
            int nowsum = 0, ans = 0;
            cout << tri.get_Kth_max_Sum(tri.root[l - 1], tri.root[r], 1, n, k,
                                        nowsum, ans)
                 << "\n";
        }
        if (ops == 3) {  // get kth max sum siz<k return allsum
            cin >> l >> r >> k;
            int nowsum = 0, ans = 0;
            tri.get_Kth_max_Sum(tri.root[l - 1], tri.root[r], 1, n, k, nowsum,
                                ans);
            cout << ans << "\n";
        }
        if (ops == 4) {  // get kth min no output -1
            cin >> l >> r >> k;
            int nowsum = 0, ans = 0;
            cout << tri.get_Kth_min_Sum(tri.root[l - 1], tri.root[r], 1, n, k,
                                        nowsum, ans)
                 << "\n";
        }
        if (ops == 5) {  // get kth min sum siz<k return allsum
            cin >> l >> r >> k;
            int nowsum = 0, ans = 0;
            tri.get_Kth_min_Sum(tri.root[l - 1], tri.root[r], 1, n, k, nowsum,
                                ans);
            cout << ans << "\n";
        }
