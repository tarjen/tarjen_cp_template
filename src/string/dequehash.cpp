...
int fpow(int n, int k, int p = mod) {int r = 1; for (; k; k >>= 1) {if (k & 1) r = r * n % p; n = n * n % p;} return r;}
...
void init()
{
    p[0] = 1; for(int i=1;i<maxn;i++) p[i] = mul(p[i - 1], base, mod);
...
struct extendable_sequence {
    ...
    void add_back(vector<int> vals) {
        ...
        for(int i=0;i<sz(vals);i++) {
            add(t, mul(vals[i], mul(p[sz(dq) - 1], ip[pow_offset], mod), mod), mod);
